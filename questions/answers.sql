-- Part 2a, Question 1
    -- For this question, I am using the matching order_id from successful checkout in events and line_items table, to get quantity of most ordered items in successful carts.
    -- product_id column is of type bigint and not UUID as stated in question description.
    ...
    SELECT
        li.item_id AS product_id, -- product_id column is of type bigint and not UUID AS stated in question description. this was pulled FROM the item_id in line_items table which is a foreign key FROM the products table
        (SELECT p.name FROM alt_school.products p WHERE p.id = li.item_id) AS product_name, -- product_name column is generated by USING a subquery to include the product name FROM the products table
        SUM(li.quantity) AS num_times_in_successful_orders -- num_times_in_successful_orders is generated by adding the number the quantities of items found in succeful checkout orders
...
-- Part 2a, Question 2
    -- For this question, I am assuming that when a customer removes an item previously added in their cart, they remove the total number added since the remove_from_cart event_type does not specify the quantity removed.
    -- I only consider items present at checkout if the item was added and never removed from the cart.

    --This CTE gets customer_ids of customers who made a successful checkout, and their event_data when they add or remove items from their cart
...
-- Part 2b, Question 1
    -- This query returns the country WITH the highest number of successful checkouts
    SELECT c.location,
...
-- Part 2b, Question 2
    --This query returns the customer id and number of events excluding visit event-types, that they performed without any successful checkouts.
    SELECT e.customer_id,
...
--Part 2b, Question 3
    -- For this question, I consider distinct visits based on their timestamp. Visits with exactly the same timestamp are considered as duplicates and counted just once

    -- This CTE extracts the customer_id and number of visits of customers before their successful checkouts
    ...
    -- Main query which calculates the average number of visits for each customer who successfully checkouts
